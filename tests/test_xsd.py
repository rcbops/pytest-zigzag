...
def test_multiple_jira_references_asc(testdir):
...
def test_multiple_jira_references_mk8s(testdir):
    """Verify that 'get_xsd' returns an XSD stream when a testcase is decorated Jira mark with multiple
...
def test_missing_required_marks_asc(testdir, undecorated_test_function):
    ...
    xml_doc = run_and_parse(testdir).xml_doc
    xmlschema = etree.XMLSchema(etree.parse(get_xsd('asc')))

    # Test
    assert xmlschema.validate(xml_doc) is False


def test_missing_required_marks_mk8s(testdir, undecorated_test_function):
...
def test_extra_testcase_property_asc(testdir, properly_decorated_test_function):
...
def test_extra_testcase_property_mk8s(testdir, properly_decorated_test_function):
...
def test_typo_property_asc(testdir, properly_decorated_test_function):
...
def test_typo_property_mk8s(testdir, properly_decorated_test_function):
